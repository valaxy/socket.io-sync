 ...
 *
 * Yes, this is copy from: https://github.com/stbrenner/socket.io-proxy
 */
...
(function () {
    ...
    var logger = require('log4js').getLogger('push');
    ...
    exports.connect = function (destinationUrl, options) {
        ...
        let reconnectUrl = 'http://localhost:' + tunnelPort + pathname +
            '?protocol=' + destination.protocol.replace(':', '') +
            '&hostname=' + destination.hostname +
            '&port=' + destination.port +
            '&query=' + encodeURIComponent(destination.query)
        return io.connect(reconnectUrl,
            options);
    };
    ...
    exports.init = function (proxyUrl) {
        ...
        if (typeof proxyUrl === 'undefined') {
            if (process.env.http_proxy) {
                ...
                logger.info('direct connection (no proxy defined)');
                return;
            }
        }
        ...
        tunnelServer = http.createServer(function (request, response) {
            ...
            for (var i in requestUrl.query) {
                if (['protocol', 'hostname', 'port', 'query'].indexOf(i) == -1) {
                    qs.push(i + '=' + requestUrl.query[i]);
                }
            }

            qs = '?' + qs.join('&') + '&' + decodeURIComponent(requestUrl.query.query);
